"""Anchor minimal overlay popup (bottom-right card).
...
_BAR_WIDTH = 530  # must match DialogAnchorPopup.xml


class AnchorPopup(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        if self._thumb:  # dimmed episode-still backdrop (Up Next), no resize
            try:
                ...
                img.setVisible(True)
            except Exception:  # noqa: BLE001 - control absent / image load failure
                pass
    ...
    def _countdown(self):
        ...
        while not self._done.is_set() and elapsed < total:
            ...
            except Exception:  # noqa: BLE001 - window torn down mid-update
                return
...
def ask(title, subtitle, primary, secondary, timeout_ms,
    ...
    win = AnchorPopup("DialogAnchorPopup.xml", ADDON.getAddonInfo("path"),
                     "default", "1080i",
